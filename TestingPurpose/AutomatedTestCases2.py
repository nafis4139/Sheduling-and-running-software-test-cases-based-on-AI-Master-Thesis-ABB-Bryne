...
def fetch_test_run_results(test_run_id, test_points):
    ...
    if response.status_code == 200:
        ...
        save_test_results_to_file(test_run_id, response_json)

        
        print("✔ Test Run Results Fetched and Saved Successfully!")
    else:
...
# Save Test Results to Log File
def save_test_results_to_file(test_run_id, response_json):
    log_dir = os.environ.get("BUILD_ARTIFACTSTAGINGDIRECTORY", ".")  # Ensure logs are stored in artifact directory
    log_filename = os.path.join(log_dir, f"TestRun_{test_run_id}_Results.log")

    # Write test results to file
    with open(log_filename, "w", encoding="utf-8") as log_file:
        log_file.write(f"Test Run ID: {test_run_id} Results\n")
        log_file.write("="*60 + "\n")
        for result in response_json.get("value", []):
            test_case_id = result.get("testCase", {}).get("id", "Unknown")
            test_case_title = result.get("testCaseTitle", "Unknown Test Case")
            agent = result.get("computerName", "Unknown Agent")
            duration = round(result.get("durationInMs", 0) / 1000, 2)
            outcome = result.get("outcome", "Unknown")
            log_file.write(f"✔ {test_case_title} (ID: {test_case_id}) - {outcome.upper()} on {agent} [{duration}s]\n")
        log_file.write("\nRaw JSON Response:\n")
    ...
    if os.path.exists(log_filename):
        print(f"##vso[artifact.upload artifactname=TestResultsLogs;]{log_filename}")
    else:
        print(f"❌ Log file {log_filename} does not exist! Check file creation process.")

    # Upload the log file to the test run in Azure DevOps
    attach_log_to_test_run(test_run_id, log_filename)


# Upload Log File to Test Run
def attach_log_to_test_run(test_run_id, log_filepath):
    file_name = os.path.basename(log_filepath)
    comment = "Log file uploaded by local script"
    attachment_type = "GeneralAttachment"

    url = (
        f"https://dev.azure.com/{AZURE_ORG}/{PROJECT}/_apis/test/Runs/{test_run_id}/attachments"
        f"?fileName={file_name}&comment={comment}&attachmentType={attachment_type}&api-version=7.1-preview.1"
    )

    headers = {
        "Authorization": f"Basic {auth_b64}",
        "Content-Type": "application/octet-stream"  # Ensure correct format
    }
    ...
    if response.status_code in [200, 201]:
        print(f"✔ Successfully uploaded log file to test run {test_run_id}")
    else:
