...
agg["failWeight"] = (agg["nFail"] / agg["nRuns"]) * 5 # Dynamically weight failure impact based on run count

agg["normalizedTime"] = (agg["avgTime"] - agg["avgTime"].min()) / (agg["avgTime"].max() - agg["avgTime"].min())
agg["priorityScore"] = (
    agg["failWeight"] +
    agg["normalizedTime"] * 1 +
    agg["lastOutcome"].apply(lambda x: 10 if x == "Failed" else 0) -
    agg["daysSinceLastRun"].apply(lambda x: 5 if x <= 1 else 0)  # Penalty for very recent run
)
